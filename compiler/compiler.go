...
// Compiler has a method Compile that takes an AST node and compiles it into bytecode
...
// Bytecode is a struct that holds the compiled bytecode.
...
func (c *Compiler) Compile(node ast.Node) error {
	...
	case *ast.IfExpression:
		...
		if c.lastInstructionIs(code.OpPop) {
			// Remove the last Pop instruction to prevent calling Pop twice(once for the consequence and once for the whole if expression and we remove the consequence's Pop instruction)
			c.removeLastPop()
		}
	...
	case *ast.HashLiteral:
		...
		// To avoid the order of the keys in the hash randomly changing, sort the keys here.
...
// Add object to constants pool and return its index
...
func (c *Compiler) addInstruction(ins []byte) int {
	// e,g, if the current instructions is [1, 2, 3], and 4 will be added to the stack then, posNewInstruction = 3
...
func (c *Compiler) setLastInstruction(op code.Opcode, pos int) {
	// Copy the last instruction to the previous instruction
	previous := c.scopes[c.scopeIndex].lastInstruction
	// Set the last instruction to the new instruction
